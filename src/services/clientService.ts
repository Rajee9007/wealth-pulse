...
export async function initClients(): Promise<Client[]> {
  try {
    const apiList = await copilotApi.fetchClients() as unknown as Client[];
    if (apiList && apiList.length > 0) {
      // The API now returns the full Client schema matching mock_clients_reference.json
      _clients = apiList;
      console.info(`[clientService] Loaded ${_clients.length} clients from live API`);
      return _clients;
    }
  } catch (err) {
